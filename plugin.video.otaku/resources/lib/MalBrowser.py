...
class MalBrowser(BrowserBase):
    ...
    def get_recently_aired(self, page=1, format=None, prefix=None):
        """
        Get recently aired shows using Simkl API for MAL provider.
        Shows TV shows whose latest episode aired in the last 7 days.
        Supports month boundary crossings via monthly calendar archives.
        """
        from datetime import datetime, timedelta, timezone
        
        # Debug logging (temporary - remove before merge)
        control.log(f"[DEBUG] Recently Aired MAL - Provider: MAL, Page: {page}", 'debug')
        
        # Compute 7-day window in UTC
        now = datetime.now(timezone.utc)
        window_start = (now - timedelta(days=6)).replace(hour=0, minute=0, second=0, microsecond=0)
        window_end = now
        
        control.log(f"[DEBUG] Time window: {window_start.isoformat()} to {window_end.isoformat()}", 'debug')
        
        # Fetch calendar data for the date range (handles month boundaries)
        simkl = Simkl()
        simkl_data = simkl.get_calendar_range(window_start, window_end)
        
        if not simkl_data:
            control.log("[DEBUG] No Simkl calendar data available", 'debug')
            return []
        
        # Debug first 10 raw Simkl items
        for idx, item in enumerate(simkl_data[:10]):
            control.log(f"[DEBUG] Raw item {idx}: simkl_id={item.get('ids', {}).get('simkl')}, "
                       f"mal_id={item.get('ids', {}).get('mal')}, "
                       f"title={item.get('title', 'N/A')}, "
                       f"type={item.get('anime_type') or (item.get('anime') or {}).get('type')}, "
                       f"date={item.get('date')}", 'debug')
        
        # Filter + collect recent episodes
        recent_episodes = []
        raw_count = len(simkl_data)
        after_window_count = 0
        after_type_count = 0
        
        for idx, item in enumerate(simkl_data):
            try:
                # Parse air timestamp with timezone awareness
                date_str = item.get('date')
                if not date_str:
                    continue
                
                air_ts = datetime.fromisoformat(date_str.replace('Z', '+00:00')).astimezone(timezone.utc)
                
                # Keep only if within 7-day window
                if not (window_start <= air_ts <= window_end):
                    continue
                ...
                # Type filter - support both anime_type and anime.type fields
                anime_type = (item.get('anime_type') or (item.get('anime') or {}).get('type') or '').lower()
                
                if format:
                    # If specific format requested, filter by it
                    format_lower = format.lower()
                    if format_lower == 'tv' and anime_type not in ('tv', 'tv_short'):
                        continue
                    elif format_lower != anime_type:
                        continue
                else:
                    # Default: TV only (tv, tv_short, ona)
                ...
                episode_number = ((item.get('episode') or {}).get('episode')) or 1
                ...
                control.log(f"[DEBUG] Error parsing Simkl item {idx}: {e}", 'debug')
                continue
        
        control.log(f"[DEBUG] Counts - raw: {raw_count}, after_window: {after_window_count}, "
                   f"after_type: {after_type_count}", 'debug')
        
        # Dedup per show - keep the latest (max air_ts) record for each mal_id
        unique_shows = {}
        for episode in recent_episodes:
            mal_id = episode['mal_id']
            if mal_id not in unique_shows or episode['air_ts'] > unique_shows[mal_id]['air_ts']:
                unique_shows[mal_id] = episode
        
        # Sort DESC by air_ts (newest first), tie-break by mal_id for stability
        sorted_episodes = sorted(unique_shows.values(), 
                               key=lambda x: (x['air_ts'], x['mal_id']), 
                               reverse=True)
        
        control.log(f"[DEBUG] Counts - unique_by_show: {len(sorted_episodes)}", 'debug')
        
        # Debug first 10 final items
        for idx, ep in enumerate(sorted_episodes[:10]):
            control.log(f"[DEBUG] Final item {idx}: mal_id={ep['mal_id']}, "
                       f"air_iso={ep['air_ts'].isoformat()}, "
                       f"ep={ep['episode_number']}", 'debug')
        ...
        control.log(f"[DEBUG] Page {page}: emitted_count={len(page_episodes)}", 'debug')
        
        # Fetch MAL details for current page
        mal_items = []
        for episode_data in page_episodes:
            mal_id = episode_data['mal_id']
            
            try:
                # Use same helper all MAL menus use
                res = database.get(self.get_base_res, 24, f"{self._BASE_URL}/anime/{mal_id}")
                ...
                # Adjust label to include episode number
                original_title = mal_item.get(self.title_lang) or mal_item.get('title') or ''
                mal_item['title'] = f"{original_title} - Episode {episode_data['episode_number']}"
                
                mal_items.append(mal_item)
                
            except Exception as e:
                control.log(f"[DEBUG] Error fetching MAL data for {mal_id}: {e}", 'debug')
                continue
        
        # Process through existing MAL artwork pipeline
        get_meta.collect_meta(mal_items)
        from functools import partial
        mapfunc = partial(self.base_mal_view, completed=self.open_completed())
        all_results = list(map(mapfunc, mal_items))
        
        # Add pagination if there are more items
        if end_idx < len(sorted_episodes):
            base_url = f"{prefix}?page=%d" if prefix else "recently_aired?page=%d"
            all_results += self.handle_paging(True, base_url, page)
